...
/// Helper function to convert sockaddr to IpAddress (Zig 0.16.0 compatibility)
...
pub const BrokerServer = struct {
    ...
    fn acceptNewClient(self: *BrokerServer, scratch: std.mem.Allocator) !void {
        ...
        if (current_clients >= self.max_clients) {
            ...
            // Convert sockaddr to IpAddress for logging
            const client_address = sockaddrToIpAddress(@alignCast(&client_addr)) catch {
        ...
        // Convert sockaddr to IpAddress for access control and logging
        // NOTE: sockaddrToIpAddress() only supports IPv4/IPv6, not Unix sockets.
        ...
        const client_address = if (is_unix_socket)
            std.Io.net.IpAddress{ .ip4 = std.Io.net.Ip4Address.loopback(0) } // Dummy localhost for Unix sockets
        else
            sockaddrToIpAddress(@alignCast(&client_addr)) catch {
                std.posix.close(client_socket);
                return BrokerError.ListenSocketError;
            };
        ...
        if (!self.access_list.isAllowed(client_address, self.config.io.?)) {
            if (!is_unix_socket) {
